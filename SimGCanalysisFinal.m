...
mspkmax = 0.07; muMax = log(mspkmax/(1-mspkmax));
mspkmin = 0.05; muMin = log(mspkmin/(1-mspkmin));
...
thetac = ones(10,1)*[1 0 2 0 0 1]; thetac = thetac(:);
...
Mhcs = 50; % Number of Self-history parameters
...
gammacv = [0.1:0.05:1]; % Choice of gamma's 
Whcv = {Whc}; Whscv = {Whcs};
...
suptitle(['Four panels of estimated J-statistics corresponding to the selected GC links'])

%% Estimate Causal maps using the alternative methods for camparison
Wml = round(W/(1-ff)); % Equiv. to Neff = W/(1-ff) of adaptive setting 
mopt = numel(thetac)/10; % Optimal selected model order
[GC1,GC2] = GCmethods(resp,Wml,mopt,WH,alpha);
...
for i = 1:Nsnp
    ...
    subplot(4,Nsnp,i), imagesc(Cmapi)
    caxis([cmin,cmax]);
end
...
for k = 1:Nsnp
    ...
    subplot(4,Nsnp,Nsnp+k), imagesc(Phik)
    caxis([cmin,cmax]);
end

%%% Plot the estimated GC maps obstained by the two alternative methods [for comparions purposes]
cmax1 = max(max(max(abs(GC1)))); cmin1 = -cmax1;
cmax2 = max(max(max(abs(GC2)))); cmin2 = -cmax2;
for k = 1:Nsnp
    Phik1 = squeeze(GC1(Tsnaps(k)/(delta*Wml),:,:));
    Phik2 = GC2(:,:,Tsnaps(k)/(delta*Wml));
    subplot(4,Nsnp,2*Nsnp+k), imagesc(Phik1), caxis([cmin1,cmax1]);
    subplot(4,Nsnp,3*Nsnp+k), imagesc(Phik2), caxis([cmin2,cmax2]);    
end
colormap jet
set(hfE,'position',[100,100,1700,600])
suptitle(['Performance comparison of the causal inference methods'])
