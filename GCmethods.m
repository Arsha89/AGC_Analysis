function [CM1,CM2,Dmltotal,wmltotal] = GCmethods(resp,Wml,mopt,WH,alpha)
...
pth = alpha*(1:Nl)/(Nl); % Benjamini-Hochberg test (Slighty different from BY rule)

GCstat = zeros(size(Dmltotal));
% Compute p-values
Pv = chi2cdf(Dmltotal,mopt,'upper');

% Compute connectivity matrices as described in Kim et al. 
for t = 1:size(Pv,1)
    Pvt = squeeze(Pv(t,:,:));
    ...
    for ee = 1:numel(siglind)
        whatee = wmltotal{rows(ee)}; 
        stee = sign(sum(whatee(t,(cols(ee)-1)*mopt+2:cols(ee)*mopt+1).*Whcml));
        GCstat(t,rows(ee),cols(ee)) = stee; %Dmltotal(t,rows(ee),cols(ee));
    end
end
CM1 = GCstat;
...
% use the same ML estimates as method by Kim
% Compute the significance levels of the LLRs + thresholding procedure

Pvth = (Pv < alpha);

CM2 = zeros(size(Pvth));
for kk = 1:size(Pvth,1)
    Pvk = squeeze(Pvth(kk,:,:));
    [ii,jj] = find(Pvk);
    Sgn = zeros(size(Pvk));
    for ll = 1:numel(ii)
